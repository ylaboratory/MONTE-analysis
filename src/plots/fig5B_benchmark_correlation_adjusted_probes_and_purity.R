...
df$method[df$method == "Unadjusted"] <- "Uncorrected"
...
g <- ggplot(df, aes(x = corr_metric_group, y = abs_metric_pearsonr, fill = method)) +
  ...
  theme_bw() +
  labs(
    x = paste0("Uncorrected probe-purity correlation (", metric,")"),
    y = paste0("Absolute Pearson correlation\n(corrected-probe vs. ", metric,")"),
    title = paste0("Corrected probe-purity correlation with ", metric," (", cancer, ")"),
    fill = "Method"
  ) +
...
pdf("results/fig5B_adjusted_probe_correlation_with_purity.pdf", width=8, height=5)
print(g)
dev.off()
